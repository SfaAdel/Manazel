  ...
  <body class="index-page ">
        ...
        <header id="header" class="container-xxl position-relative p-0">
            <nav class="navbar navbar-expand-lg navbar-light m-auto text-center px-4 px-lg-5 py-3 py-lg-0" id="navbar">
                <a href="{{ route('home') }}" class="navbar-brand p-0" title="منازل">
                ...
                <div class="collapse navbar-collapse mr-4 justify-content-center" id="navbarCollapse">
                    <div class="navbar-nav ms-auto py-0">
                        <a href="{{ route('home') }}"
                            class="nav-item mx-3 mt-1 nav-link {{ Request::is('home') ? 'active' : '' }}" title="الرئيسية">الرئيسية</a>
                        <a href="{{ route('about') }}"
                            class="nav-item mt-1 nav-link {{ Request::is('about') ? 'active' : '' }}" title="من نحن">من نحن</a>
                        <div class="nav-item dropdown">
                            <a href="{{ route('categories') }}"
                                class="nav-link dropdown-toggle {{ Request::is('services') ? 'active' : '' }}"
                                id="servicesDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false" title="خدماتنا">
                                خدماتنا
                                <i class="bi bi-caret-down mr-1"></i>
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="servicesDropdown">
                                <li><a class="dropdown-item" href="{{ route('categories', ['slug' => 'عرض_الكل']) }}" title="عرض الكل"> عرض الكل </a></li>

                                @foreach ($navCategories as $navCategory)
                                    <li><a class="dropdown-item" href="{{ route('services', ['id' => $navCategory->id, 'name' => $navCategory->name]) }}" title="{{ $navCategory->name }}"> {{ $navCategory->name }} </a></li>
                                @endforeach
                            </ul>
                        </div>
                        <a href="{{ route('blogs') }}"
                            class="nav-item mt-1 nav-link {{ Request::is('blogs') ? 'active' : '' }}" title="المدونة">المدونة</a>
                        <a href="{{ route('contact') }}"
                            class="nav-item mt-1 nav-link {{ Request::is('contact') ? 'active' : '' }}" title="تواصل معنا">تواصل معنا</a>
                    </div>
                    <a href="{{ route('provider_form') }}" class="btn-getstarted" title="اشترك كمزود خدمة"> اشترك كمزود خدمة </a>

                    @if (!(auth()->guard('customer')->check()))
                    <a class="btn-getstarted m-3 d-block"  href="{{ route('login') }}" title="تسجيل الدخول"> تسجيل الدخول </a>
                    @else
                    <div class="dropdown">
                        <button class="btn-getstarted mx-2 dropdown-toggle" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" title="الصفحة الشخصية">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <li><a class="dropdown-item" href="{{ route('customer_edit') }}" title="تعديل الحساب الشخصي">تعديل الحساب الشخصي</a></li>
                            <li><a class="dropdown-item" href="{{ route('customer_orders') }}" title="متابعة الطلبات"> متابعة الطلبات</a></li>
                            <li>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="dropdown-item" title="تسجيل الخروج">تسجيل الخروج</button>
                                </form>
                            </li>
                        </ul>
                    </div>
